...
class PackageDocuments(models.Model):
    ...
    level_education = models.ForeignKey(
        "reference.LevelEducation",
        verbose_name=_("Уровни образования"),
        on_delete=models.CASCADE
    )
    qualification = models.ForeignKey(
        "reference.Qualification",
        verbose_name=_("Код и наименование ООП"),
        on_delete=models.CASCADE
    )
    teacher_full_name = models.CharField(
        _("ФИО научного руководителя"),
        max_length=500
    )
    ...
    review = models.FileField(
        _("Файл отзыва"),
        validators=[FileExtensionValidator(["pdf","jpg"])]
    )
    graduation_work = models.FileField(
        _("Файл ВКР"),
    ...
    update_date = models.DateTimeField(
        _("Дата и время последнего обновления"),
        auto_now=True
        )
